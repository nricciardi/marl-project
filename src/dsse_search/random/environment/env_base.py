...
class DroneSwarmSearchBase(ABC, ParallelEnv):
    def __init__(
        ...
        if self.drone.amount > self.grid_size * self.grid_size:
            raise ValueError(
                f"There are more drones than grid spots. Reduce number of drones or increase grid size (provided: {self.drone.amount})."
            )
    ...
    def required_drone_positions(self, drones_positions: list):
        if len(drones_positions) != len(self.possible_agents):
            raise ValueError(
                ...
                "please make sure there are the same number of initial positions "
